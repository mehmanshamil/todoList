...
function todoLengthFunction() {
    let todoLength = document.getElementById("todoLength")
    todoLength.innerHTML = ""
    let todo = JSON.parse(localStorage.getItem("todo")) || []
    todoLength.innerHTML = `${todo.length} / ${countTotal}`
}
todoLengthFunction()
...
function addClickListenersToItems() {
    ...
    li.forEach((item) => {
        item.addEventListener("click", () => liFunc(item));
    });
}



function liFunc(item) {
...
function searchItem() {
    let todo = JSON.parse(localStorage.getItem("todo")) || []
    let result = todo.filter((item) => item.includes(todoSearch.value))
    if (result.length > 0) {
        visiableItem(result)
    } else {
        listgrop.innerHTML = " <li  class ='list-group-item bg-danger text-light' > Melumat tapilmadi ! </li>"
    }
